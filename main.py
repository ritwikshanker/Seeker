...
def res(status):
    """Returns the status of the episode."""
...
def check_date():
    """Compares present date with the episode's date. """
    try:
        for j in range(no_Episode):  # running the loop for all the episodes
            ...
            global flag
            if 'raw release dates' in episode_Release_date[
                'data']:  # Checking if episode's release date info is present ot not.
...
if __name__ == '__main__':
    ...
    for i in range(len(series_Name)):
        try:
            ...
            print(e)
    sendmailfunc.sendMail(message_to_send, str(email_Add))  # Sending Mail to customer
    flag = -1
